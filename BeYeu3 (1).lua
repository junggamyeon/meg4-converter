...
local function autoFeed()
    ...
    FEED_TARGETS = FEED_TARGETS or {}

    table.sort(bees, function(a, b)
        return a.level > b.level
    end)

    local function isInTargets(b)
        for _, t in ipairs(FEED_TARGETS) do
            if t.col == b.col and t.row == b.row then
                return true
            end
        end
        return false
    ...
    if #FEED_TARGETS < maxCount then
        for _, b in ipairs(bees) do
            if b.level < targetLevel and not isInTargets(b) then
